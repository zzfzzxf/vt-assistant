  ...
  return <>
    ...
    <main id="main">
      ...
      <section className="container download-section" id="download">
        <div className="download-surface" data-reveal><div className="download-monogram" aria-hidden="true">VT</div><div className="download-copy"><div className="eyebrow">YOUR NEXT LEVEL STARTS HERE</div><h2>实力就位。<br />现在，轮到你了。</h2><p>让 VT助手，成为你的下一位游戏搭档。</p></div><div className="download-action"><button className="button button-primary download-button" onClick={startDownload} disabled={download.state === 'loading'}>{download.state === 'loading' ? <><CircleNotch size={21} className="spinner" /> 正在下载 {download.percent}%</> : <>下载软件 <DownloadSimple size={22} /></>}</button><span className={`download-feedback ${download.state === 'error' ? 'download-error' : ''}`} role="status" aria-live="polite">{download.message || '下载后解压，按照包内说明开始使用。'}</span></div></div>
      </section>
